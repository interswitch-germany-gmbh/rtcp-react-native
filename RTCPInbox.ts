...
class RTCPInbox extends RTCPEvents {
    logPrefix: string = "[RTCP Inbox]"; // string to append to log
    log: typeof RTCP.log = RTCP.log; // use logger of main module

    private _inbox: Notification[] = []; // initialize empty inbox
    private _inboxReady: boolean = false; // to be able to wait for inbox to be loaded on app start
    private _lastInboxSync: Date = new Date(0); // timestamp of last inbox sync with server

    private _readReceiptQueue: string[] = []; // array of push_ids to send read receipt to server for
    private _readReceiptTimer: ReturnType<typeof setTimeout> | null = null; // reference to timer object

    protected _events: string[] = ["onInboxUpdate"]; // list of subscribable events
    ...
    async init(options?: InboxOptions): Promise<void> {
        this.log("Initializing rtcp-react-native Inbox module");
        this.log("RTCPApi appid is: " + RTCP.appID());

        // --- Module configuration ---

        // enableBadge
        this.enableBadge = options?.enableBadge !== undefined ? options.enableBadge : true;
        ...
        if (this.enableBadge) this.registerEventHandler("onInboxUpdate", (inbox: Notification[], fromStorage: boolean) => {
            notifee.setBadgeCount(this.getUnreadCount());
            if (!fromStorage) this._writeInboxToStorage(); // don't write to storage if it has just been loaded from
        });

        // inboxSize
        this.inboxSize = options?.inboxSize !== undefined ? options.inboxSize : DEFAULTINBOXSIZE;
        if (Platform.OS === "ios") await DefaultPreference.set("rtcp_inbox_size", this.inboxSize.toString()); // store in userdefaults for NSE

        // syncOnAppstart
        this.syncOnAppstart = options?.syncOnAppstart !== undefined ? options.syncOnAppstart : false;
        ...
        // --- Initializations ---
        RTCP.registerEventHandler("onRemoteNotification", (notification: { data: Notification; foreground?: boolean }) => this._onRemoteNotification(notification));
        RTCP.registerEventHandler("onChangeAppID", () => this._loadInboxFromStorage());
    }

    async syncInbox(force: boolean = false): Promise<void> {
        if (force || new Date().getTime() - this._lastInboxSync.getTime() > SYNCTIMEOUT * 1000) {
    ...
    setRead(index: number): void {
        if (this._inbox[index] && this._inbox[index]["read"] === false) {
            ...
            if (this._readReceiptQueue.indexOf(this._inbox[index]["push_id"]) === -1) {
                this._readReceiptQueue.push(this._inbox[index]["push_id"]);
            }
    ...
    private async _onRemoteNotification(notification: { data: Notification; foreground?: boolean }): Promise<void> {
        ...
        if (Platform.OS === "android") {
            ...
            const isForeignInbox = data.app_id && data.app_id !== RTCP.appID();
            let inbox: Notification[] = isForeignInbox
                ? (JSON.parse(await DefaultPreference.get("rtcp_inbox_" + data.app_id) ?? '[]') || [])
                : this._inbox;
            ...
            if (data.revoke || (data.message && !data.not_in_inbox)) {
                ...
                if (data.revoke) {
                    const index = inbox.findIndex((n) => n.push_id === data.revoke);
                    if (index >= 0) {
                    ...
                    const index = inbox.findIndex((n) => n.push_id === data.replace);
                    if (index >= 0) {
    ...
    private async _loadInboxFromStorage(): Promise<void> {
        RTCP.enableLogging && this.log("Loading Inbox from storage");

        // get inbox and last sync time from storage
        const inboxString: string | null | undefined = await DefaultPreference.get("rtcp_inbox_" + RTCP.appID());
        if (inboxString) {
            this._inbox = JSON.parse(inboxString) as Notification[];
            this._lastInboxSync = new Date(await DefaultPreference.get("rtcp_last_inbox_sync_" + RTCP.appID()) ?? 0);
            this._inboxReady = true;
    ...
    private async _writeInboxToStorage(): Promise<void> {
        RTCP.enableLogging && this.log("Writing Inbox to storage");
    ...
    private _startReadReceiptTimer(delay: number): void {
        // stop existing timer
        if (this._readReceiptTimer !== null) {
            clearTimeout(this._readReceiptTimer);
            this._readReceiptTimer = null;
            if ((AppState as any).removeEventListener) (AppState as any).removeEventListener("change", this._sendReceiptsOnAppClose); // RN <=0.64 compatibility
    ...
    private _sendReceiptsOnAppClose = (nextAppState: AppStateStatus): void => {
