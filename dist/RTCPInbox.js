...
class RTCPInbox extends RTCPEvents {
    ...
    inboxSize;
    syncOnAppstart;
    appStateChangeEventSubscription;
    async init(options) {
        ...
        if (Platform.OS === "ios")
            await DefaultPreference.set("rtcp_enable_badge", this.enableBadge.toString()); // store in userdefaults for NSE
        // update badge and write inbox to storage on changes
        if (this.enableBadge)
            this.registerEventHandler("onInboxUpdate", (inbox, fromStorage) => {
                notifee.setBadgeCount(this.getUnreadCount());
                if (!fromStorage)
                    this._writeInboxToStorage(); // don't write to storage if it has just been loaded from
            });
        // inboxSize
        this.inboxSize = options?.inboxSize !== undefined ? options.inboxSize : DEFAULTINBOXSIZE;
        if (Platform.OS === "ios")
            await DefaultPreference.set("rtcp_inbox_size", this.inboxSize.toString()); // store in userdefaults for NSE
        // syncOnAppstart
        ...
        if (Platform.OS === "android") {
            ...
            if (data.revoke || (data.message && !data.not_in_inbox)) {
                ...
                if (data.revoke) {
                    ...
                    inbox.unshift(data);
                    if (inbox.length > this.inboxSize)
                        inbox.length = this.inboxSize; // limit inbox to max size
                }
                if (isForeignInbox)
                    await DefaultPreference.set("rtcp_inbox_" + data.app_id, JSON.stringify(inbox));
                else
                    this._emitEvent("onInboxUpdate", this._inbox);
            }
        }
    ...
    async _loadInboxFromStorage() {
        ...
        const inboxString = await DefaultPreference.get("rtcp_inbox_" + RTCP.appID());
        if (inboxString) {
    ...
    _startReadReceiptTimer(delay) {
        ...
        if (this._readReceiptTimer !== null) {
            ...
            if (AppState.removeEventListener)
                AppState.removeEventListener("change", this._sendReceiptsOnAppClose); // RN <=0.64 compatibility
            else
...
//# sourceMappingURL=RTCPInbox.js.map